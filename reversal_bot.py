...
def get_token_price(token_address):
    """Fetch current token price from DexScreener API - supports Solana (SOL) and BSC (USD)"""
    ...
    try:
        ...
        if data.get("pairs") and len(data["pairs"]) > 0:
            ...
            # Try BSC if no Solana pair found
            if not best_pair:
                best_volume = -1
                for pair in pairs:
            ...
            if best_pair:
                # Solana: use native price (SOL), BSC: use USD price
                if chain == "solana":
...
def handle_command(chat_id, user_id, text):
    ...
    # /add CA PERCENT - Add new token
        ...
        price_unit = token_info.get("price_unit", "SOL")
        chain_name = "Solana" if token_info.get("chain") == "solana" else "BSC"
