...
MAX_DROP_PERCENT = 50  # Ignore price drops larger than this (API glitch protection)
...
def get_token_price(token_address):
    """Fetch current token price from DexScreener API - picks highest liquidity pool"""
    ...
    try:
        ...
        if data.get("pairs") and len(data["pairs"]) > 0:
            # Sort by liquidity and pick the highest
            pairs = data["pairs"]
            best_pair = None
            best_liquidity = 0
    ...
    while True:
        try:
            ...
            for ca, data in list(tokens.items()):
                ...
                # Protection: ignore sudden drops > MAX_DROP_PERCENT (API glitch)
                if local_bottom > 0:
                    drop_percent = ((local_bottom - current_price) / local_bottom) * 100
                    if drop_percent > MAX_DROP_PERCENT:
                        print(f"⚠️ {data['symbol']}: Ignoring suspicious drop of {drop_percent:.1f}% (likely API glitch)")
                        continue
