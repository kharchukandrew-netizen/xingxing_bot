...
def get_token_price(token_address):
    """Fetch current token price from DexScreener API - picks pool with highest hourly volume, returns SOL price"""
        ...
        if data.get("pairs") and len(data["pairs"]) > 0:
            ...
            if not best_pair:
                ...
                for pair in pairs:
                    ...
                    if volume_h6 > best_volume_h6:
                        best_volume_h6 = volume_h6
                        best_pair = pair
            
            # Final fallback: just take first SOL pair with matching base token
            if not best_pair:
                for pair in pairs:
                    if pair.get("baseToken", {}).get("address", "").lower() != token_address.lower():
                        continue
...
def send_pushover_alert(token_info, ca, percent_gain, local_bottom):
    ...
    payload = {
        ...
        "message": (
            ...
            f"💰 Поточна ціна: {token_info['price']:.10f} SOL\n"
            f"📉 Локальне дно: {local_bottom:.10f} SOL\n"
...
def handle_command(chat_id, user_id, text):
    ...
    # /add CA PERCENT - Add new token
        ...
        msg = (
            ...
            f"💰 Current price: {token_info['price']:.10f} SOL\n"
    ...
    # /list - Show all tokens
        ...
        for ca, data in tokens.items():
            ...
            msg += (
                f"<b>{data['symbol']}</b>\n"
                f"  📉 Bottom: {bottom:.10f} SOL\n"
                f"  💰 Now: {current_price:.10f} SOL ({status})\n"
        ...
        try:
            ...
            for ca, data in list(tokens.items()):
                ...
                if current_price < local_bottom:
                    ...
                    save_tokens()
                    print(f"📉 {data['symbol']}: New bottom {current_price:.10f} SOL")
                    continue
                ...
                if local_bottom > 0:
                    ...
                    if percent_gain >= target_percent:
                        ...
                        print(f"📊 {data['symbol']}: {current_price:.10f} SOL (+{percent_gain:.1f}% from bottom, target: +{target_percent}%)")
