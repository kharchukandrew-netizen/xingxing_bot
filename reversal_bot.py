...
Uses SOL price (priceNative) instead of USD.
"""
...
def get_token_price(token_address):
    """Fetch current token price from DexScreener API - supports Solana (SOL), ETH/BSC/Robinhood (USD)"""
    ...
    try:
        ...
        if data.get("pairs") and len(data["pairs"]) > 0:
            # Only keep pairs where our token is the base token
            pairs = [
            ...
            # Try each supported chain in priority order
            for target_chain in ["solana", "ethereum", "robinhood", "bsc", "base"]:
                for pair in pairs:
                    if pair.get("chainId") != target_chain:
                        continue
                    volume_h1 = pair.get("volume", {}).get("h1", 0) or 0
                    if volume_h1 > best_volume:
                        best_volume = volume_h1
                        best_pair = pair
                        chain = target_chain
                
                # Fallback: any pair on this chain if no volume
                if not best_pair:
                    for pair in pairs:
                        if pair.get("chainId") == target_chain:
                            best_pair = pair
                ...
                # If found a pair on this chain, stop looking
                if best_pair:
                    break
            
            # Final fallback: take highest volume pair on any chain
            if not best_pair:
                for pair in pairs:
                    volume_h1 = pair.get("volume", {}).get("h1", 0) or 0
                    if volume_h1 > best_volume:
            ...
            if best_pair:
                # Solana: use native price (SOL), others: use USD price
                if chain == "solana":
...
def send_pushover_alert(token_info, ca, percent_gain, local_bottom, price_unit):
    ...
    payload = {
        ...
        "message": (
            ...
            f"💰 Поточна ціна: {token_info['price']:.10f} {price_unit}\n"
            f"📉 Локальне дно: {local_bottom:.10f} {price_unit}\n"
...
def handle_command(chat_id, user_id, text):
    ...
    # /add CA PERCENT - Add new token
        ...
        if not token_info:
            send_telegram_message(chat_id, "❌ Token not found on DexScreener or no SOL pair available. Check the CA.")
            return
        ...
        tokens[ca] = {
            ...
            "price_unit": token_info.get("price_unit", "SOL")
        }
        save_tokens()
        ...
        msg = (
            ...
            f"⛓ Chain: {chain_name}\n"
            f"💰 Current price: {token_info['price']:.10f} {price_unit}\n"
    ...
    # /list - Show all tokens
        ...
        for ca, data in tokens.items():
            ...
            price_unit = data.get("price_unit", "SOL")
            ...
            msg += (
                ...
                f"  📉 Bottom: {bottom:.10f} {price_unit}\n"
                f"  💰 Now: {current_price:.10f} {price_unit} ({status})\n"
        ...
        try:
            ...
            for ca, data in list(tokens.items()):
                ...
                if current_price < local_bottom:
                    ...
                    save_tokens()
                    print(f"📉 {data['symbol']}: New bottom {current_price:.10f} {price_unit}")
                    continue
                ...
                if local_bottom > 0:
                    ...
                    if percent_gain >= target_percent:
                        ...
                        # Send Pushover alert
                        send_pushover_alert(token_info, ca, percent_gain, local_bottom, price_unit)
                        
                        # Mark for removal
                        tokens_to_remove.append(ca)
                    else:
                        print(f"📊 {data['symbol']}: {current_price:.10f} {price_unit} (+{percent_gain:.1f}% from bottom, target: +{target_percent}%)")
