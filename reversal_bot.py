...
def get_token_price(token_address):
    """Fetch current token price from DexScreener API - supports Solana (SOL), BSC (USD), ETH (USD)"""
    ...
    try:
        ...
        if data.get("pairs") and len(data["pairs"]) > 0:
            ...
            # First, try to find Solana SOL pair
            for pair in pairs:
            ...
            # Try ETH if no Solana pair found
            if not best_pair:
                best_volume = -1
                for pair in pairs:
            ...
            # Try BSC if no Solana or ETH pair found
            if not best_pair:
                best_volume = -1
                for pair in pairs:
            ...
            if best_pair:
                # Solana: use native price (SOL), BSC/ETH: use USD price
                if chain == "solana":
